...
function transformIfPlaceholder(transform, transformReady, generateText) {
...
const transformReadyFunction = async (response, imageURI, log, transform, upload, originalImageURI) => {
...
function processImage(config) {
	...
	return (request, response, next) => {
		...
		if (imageUploadCache[originalImageURI]) {
			...
			transformIfPlaceholder(transform, transformReady, generateText);
		} else {
			...
			transformReady = axios
				...
				.then(async response =>
					await transformReadyFunction(response, imageURI, log, transform, upload, originalImageURI)
					,
					error => {
						const newError = createErrorFromAxiosError(error, imageURI);
						throw newError;
