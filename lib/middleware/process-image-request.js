...
function processImage(config) {
	...
	return (request, response, next) => {
		...
		if (
			...
			transform.setUri(
				`${protocol}://${hostname}${hostname === 'localhost' ? ':8080' : ''}/__origami/service/image/v2/images/svgtint/${encodedUri}${hasQueryString ? '&' : '?'
				}color=${tint}`
			);
		...
		if (imageUploadCache[originalImageURI]) {
			...
			.get(imageURI, {
				...
				headers: {
					'range': 'bytes=0-1000',
					'User-Agent': 'Origami Image Service Service' + Math.random(),
				}
			}).then(async response => {
				...
				return axios
					.get(imageURI, {
						...
						responseType: 'arraybuffer',
						headers: {
							'User-Agent': 'Origami Image Service' + Math.random(),
						}
					})
					.then(
						async (imageResponse) => {
