...
function processImage(config) {
	...
	return (request, response, next) => {
		...
		if (
			...
			transform.setUri(
				`${protocol}://${hostname}/v2/images/svgtint/${encodedUri}${
					hasQueryString ? '&' : '?'
				}color=${tint}`
			);
		...
		if (cache[originalImageURI]) {
			...
			transform.type = 'text';
			if (cache[info]) {
				request.placeholderUri = cache[info];
			} else {
				...
				// this is a little padding to make the text
				// readable on flagrantly unsquare images
				...
				transformReady = generateText(info + padding, {
					...
					cache[imageName] = cloudinaryText.secure_url;
					request.placeholderUri = cloudinaryText.secure_url;
				});
			}
		} else {
			transformReady = axios
				.get(encodeURI(originalImageURI), {
