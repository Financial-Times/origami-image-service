	...
	return async (request, response, next) => {
		...
		if (!/^\p{Letter}+(-\p{Letter}+)+$/u.test(name)) {
			const error = httpError(
				404,
				'Refusing to get image for the provided name - The image name was an unexpected format. If this name should work, please contact someone from the Origami team.'
			);
		...
		if (cmsURL) {
			...
			try {
				const capi = `https://api.ft.com/concepts?type=http://www.ft.com/ontology/person/Person&mode=search&q=${encodeURIComponent(
					name
				)}&boost=authors`;
				const conceptSearchResponse = await ftApiRequest(
					capi,
				...
				if (conceptSearchResponse.status >= 500) {
					const error = httpError(
						500,
						`Unable to get image for ${name} - The url "${capi}" responded with a ${conceptSearchResponse.status} status code.`
					);
				...
				const concepts =
					(conceptSearchResponse.data && conceptSearchResponse.data.concepts) ||
					[];
				const author = findAuthor(concepts, name);
				if (!author) {
					const error = httpError(
						404,
						`Unable to get image for ${name} - No author found who matches that name.`
					);
				...
				const peopleResponse = await ftApiRequest(
					author.apiUrl,
					config.contentApiKey
				);
				if (peopleResponse.status >= 500) {
					const error = httpError(
						500,
						`Unable to get image for ${name} - The url "${author.apiUrl}" responded with a ${peopleResponse.status} status code.`
					);
					error.cacheMaxAge = '5m';
					error.skipSentry = true;
					throw error;
				...
				if (!imageUrl) {
					const error = httpError(
						404,
						`Unable to get image for ${name} - The author has no headshot.`
					);
					// There are applications which use the fthead scheme as a way to check if a person has a headshot.
