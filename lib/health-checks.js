...
const healthChecks = module.exports = {
	...
	init(config) {
		...
		customSchemeStoreBucket = config.customSchemeStore;
	...
	customSchemeStore: {
		getStatus: () => ({
			...
			panicGuide: `Check that the bucket ${customSchemeStoreBucket} still exists and that the AWS Region is up`
		})
	}
};
