		...
		checks: [
			...
			// on a non-200 response
			{
				...
				url: () => {
					...
					return cloudinary.url(`${options.customSchemeStore}/__origami/service/image/internal-images/1x1.gif?cachebust=${cacheBust}`, {type: 'fetch'});
				},
				interval: 60000,
				...
				headers: {
					'User-Agent': 'FTSystem/origami-image-service-v2 (healthcheck)'
				}
			},
			...
			// response
			{
				...
				url: () => {
					...
					return `${customSchemeBaseUrl}/__origami/service/image/internal-images/fticon/v1/cross.svg`;
				},
				interval: 30000,
				...
				panicGuide: `Check that ${options.customSchemeStore}/__gtg is responding with a 200 status and that Heroku's status is ok.`,
				headers: {
					'User-Agent': 'FTSystem/origami-image-service-v2 (healthcheck)'
				}
			},
