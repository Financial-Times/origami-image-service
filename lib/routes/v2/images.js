...
module.exports = app => {
	...
	function mapParams(request, response, next) {
		...
		if (schemeParam) {
			...
			if (request.params.schemeUrl.match(/^(https?)(:|%3A)?(\/|%2F)([^/])/)) {
				request.params.schemeUrl = request.params.schemeUrl.replace(
					/^(https?)(:|%3A)?(\/|%2F)([^/])/,
					'$1$2//$4'
				);
			}
	...
	function updateOldBlogsLinks(request, response, next) {
		if (request.params.schemeUrl.startsWith('https://blogs.r.ftdata.co.uk/')) {
			request.params.schemeUrl = request.params.schemeUrl.replace(
				'https://blogs.r.ftdata.co.uk',
				'https://blogs.ft.com'
			);
			request.params.imageUrl = request.params.schemeUrl;
		} else if (
			request.params.schemeUrl.startsWith('http://blogs.r.ftdata.co.uk/')
		) {
			request.params.schemeUrl = request.params.schemeUrl.replace(
				'http://blogs.r.ftdata.co.uk',
				'https://blogs.ft.com'
			);
			request.params.imageUrl = request.params.schemeUrl;
		}
	...
	function handleImage(request, response, next) {
		switch (request.params.imageMode) {
			...
			case 'purge':
				response.redirect(
					`/__origami/service/image/v2/purge/url/?url=${encodeURIComponent(
	...
	app.get(
		/^\/__origami\/service\/image\/v2\/images\/(raw|debug|metadata|purge|placeholder)\/((ftflag|fticon|ftsocial|ftlogo|ftbrand|app-badge|ftpodcast|specialisttitle)(-v\d+)?(:|%3A).*)$/i,
		mapParams,
		mapScheme,
		mapCustomScheme(app.ft.options),
		requireValidSourceParam,
	...
	app.get(
		...
		(request, response, next) => {
			...
			if (hostName && hostnameBlocklist.includes(hostName)) {
				const error = httpError(
					403,
					`Images from "${hostName}" are blocked. Please contact the Origami team within Slack (#origami-support) with questions.`
				);
	...
	app.get(
		...
		st({
			path: path.join(__dirname, '../../../image-sets/'),
			url: '/internal-images/',
		})
	);

	app.get(
		'/__origami/service/image/v2/imagesets/:scheme(ftbrand|ftbrand-v1|app-badge|app-badge-v1|ftflag|ftflag-v1|fticon|fticon-v1|ftlogo|ftlogo-v1|ftpodcast|ftpodcast-v1|ftsocial|ftsocial-v1|ftsocial-v2|specialisttitle|specialisttitle-v1)',
		(request, response) => {
