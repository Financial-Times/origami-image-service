...
module.exports = app => {
  const metrics = app.ft.metrics;

	const requireValidSourceParam = sourceParam({
		verifyUsingCmdb: false
	});

  // extract hostname from image url using regex
  function extractHostname(url) {
    const regex = /https?:\/\/(.+)\/.+/gm;
    // The regex has capturing group and will capture anything between http(s):// and first / after that.
    ...
    async (request, response, next) => {
      const hostName = extractHostname(request.params.imageUrl);
      // Send one request to monitoring to increase the total number
      metrics.count('hostnames.total', 1);
      // sets Redis set which has only unique values
			const reply = await app.redisClient.sendCommand(['SADD', 'hostnames', hostName]);
      // If the value is 0 it means that the hostname is already in the set
      // Send another request to monitoring to increase the hit number
      if (reply === 0) {
        metrics.count('hostnames.hit', 1);
      }
      next();
		},
		processImageRequest(app.ft.options),
