...
module.exports = app => {
	...
	function mapParams(request, response, next) {
		...
		if (schemeParam) {
			...
			// we should never get here, because of the earlier middleware
			throw new Error('Empty image URI only valid for placeholder images');
		}
		next();
	}
	...
	app.get(
		/^\/v2\/images\/(raw|debug|metadata|purge|placeholder)\/((ftflag|fticon|fthead|ftsocial|ftpodcast|ftlogo|ftbrand|specialisttitle)(-v\d+)?(:|%3A).*)$/i,
		mapParams,
		mapScheme,
		mapCustomScheme(app.ft.options),
		requireValidSourceParam,
