...
module.exports = app => {
	...
	// v2 url-builder page
	app.get('/__origami/service/image/v2/docs/image-sets', cacheControl({maxAge: '7d'}), (request, response) => {
		...
		response.render('image-sets', {
			...
			imagesets: [
				...
				},
				{
					name: 'App Badge',
					id: 'App-Badge',
					images: Object.entries(appBadge).map(([name, metadata]) => {
						return {
							name,
							deprecated: metadata.deprecated,
							uri: 'app-badge-v1:' + name
						};
					})
				},
				{
