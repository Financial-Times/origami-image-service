...
const flags = require('@financial-times/origami-flag-images').images.reduce(function(accumulator, currentValue) {
		...
		switch (scheme) {
			case 'ftflag': {
				switch (version) {
					case 'v1': {
						if (flags[pathname]) {
							return flags[pathname] + queryString;
						} else {
							throw new Error(schemeErrorMessage);	
						}
					}
			...
			// Non-SVG-based schemes. Here we're not specifying
			// an extension
			case 'ftbrand':
			case 'fthead':
			case 'ftpodcast':
