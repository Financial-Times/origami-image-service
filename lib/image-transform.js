...
module.exports = class ImageTransform {
	...
	constructor(properties) {
		...
		this.setDpr(properties.dpr, properties.width, properties.height);
	 ...
	 */
	setDpr(value, width, height) {
		let errorMessage = 'Image DPR must be a positive whole number';
		this.dpr = ImageTransform.sanitizeNumericValue(value, errorMessage);
		if(this.dpr > 1) {
			if (height) {
				this.setHeight(Math.floor(height / this.dpr));
			}
			if (width) {
				this.setWidth(Math.floor(width / this.dpr));
			}
			if(!width && !height) {
				errorMessage = 'Image DPR can only be set when the transform has an explicit width or height';
