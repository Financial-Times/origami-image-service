			...
			case 'fticon':
				{
					switch (version) {
						case 'v1':
							{
								if (fticonV1[pathnameWithoutExtension]) {
									return `${baseUrl}/internal-images/fticon/v1/${fticonV1[pathnameWithoutExtension]}${queryString}`;
								} else {
									throw new Error(schemeErrorMessage);
								}
							}
						default:
							{
								return `${baseUrl}/internal-images/fticonold/v4/${fticon[pathnameWithoutExtension]}${queryString}`;
							}
					}
				}
			case 'ftlogo':
