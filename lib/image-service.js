...
function createProxy(errorHandler) {
	...
	function proxyResponseHandler(proxyResponse, request, response) {
		...
		const normaliseKeys = key => base64.encode((key || '').toLowerCase());

		proxyResponse.headers['Surrogate-Key'] = `${keyForAllImages} ${normaliseKeys(keyForImageType)} ${normaliseKeys(keyForScheme)} ${normaliseKeys(request.params.schemeUrl)}`;
