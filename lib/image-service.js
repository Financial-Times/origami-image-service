...
function createProxy(errorHandler) {
	...
	function proxyResponseHandler(proxyResponse, request, response) {
		...
		const normaliseKeys = key => base64.encode((key || '').replace(/[^a-z0-9-]/gi, '').toLowerCase());

		proxyResponse.headers['Surrogate-Key'] = `${normaliseKeys(keyForAllImages)} ${normaliseKeys(keyForImageType)} ${normaliseKeys(keyForScheme)} ${normaliseKeys(request.params.schemeUrl)}`;
