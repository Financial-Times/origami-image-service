...
const origamiImageServiceSurrogateKey = require('./middleware/origami-image-service-surrogate-key');
const supressFTComErrorPage = require('./middleware/origami-image-service-supress-ft-com-error-page.js');
const timingAllowOrigin = require('./middleware/origami-image-service-timing-allow-origin');
...
function imageService(options) {
	...
	options.sentryConfig = {
		shouldSendCallback: sentryShouldSend
	};

	const app = origamiService(options);
	app.proxy = createProxy(origamiService.middleware.errorHandler());
  app.workerQueue = createRedisQueue();

	app.use(supressFTComErrorPage());
...
function createRedisQueue() {
  ...
  const workerQueue = new Queue("hostnames", REDIS_URL)
  return workerQueue;
}
...
function createProxy(errorHandler) {
	...
	function proxyResponseHandler(proxyResponse, request, response) {
		...
		proxyResponse.headers['Vary'] = 'Content-Dpr, Accept';
	}
...
// Function to work out whether sentry should
// send an error
function sentryShouldSend(data) {
	const errorCode = (data.extra.Error ? data.extra.Error.code : null);
	...
	if (/^BadGatewayError:/.test(data.message)) {
		return false;
	}
	return true;
}
